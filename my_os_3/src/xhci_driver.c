...
struct DeviceDescriptor {
    ...
    //for example 0x0200 is usb 02.00
        release_bcd_maj, release_bcd_min,
...
static struct DeviceDescriptor send_control_transfer(struct xHCIData *xhci, uint8_t slot_id, struct Ring *ep0_transfer, uint64_t num_bytes) {
    ...
    //data stage
    uint64_t data_buffer_phys = malloc4k_phys();
    struct DeviceDescriptor *data_buffer_virt = phys_to_hhdm(data_buffer_phys);
    ...
    // enqueue_ring(ep0_transfer, (struct TRB) {
    //     .parameter.raw = 0xDEADDEADDEADDEAD,
    //     .status.event_data = {
    //         .interrupt_target = 0,
    //         .interrupt_on_completion = 1,
    ...
    // assert(recv.parameter.raw == 0xDEADDEADDEADDEAD);

    // recv = extract_from_list(xhci->unhandled_events, TRB_TYPE_TRANSFER);
    // assert(recv.status.type_transfer.trb_type != 0);
    // assert(recv.status.type_transfer.completion_code == 1);
    // assert(recv.status.type_transfer.slot_id == slot_id);
    ...
    struct DeviceDescriptor result = *data_buffer_virt;
    free4k_phys(data_buffer_phys);
    return result;
}
...
static void initialise_port(struct BarInfo bar, struct xHCIData *xhci, uint64_t *dcbaa_virt, uint8_t port_idx) {
    ...
    struct DeviceDescriptor data_buffer = send_control_transfer(xhci, slot_id, ep0_transfer, 8);
    assert(device_context->slot_state = 2);
    ...
    data_buffer = send_control_transfer(xhci, slot_id, ep0_transfer, data_buffer.length);
